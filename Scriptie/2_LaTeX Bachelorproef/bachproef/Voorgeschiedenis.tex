...
\begin{itemize}
    ...
    \item Subnetten opkuisen: vb. \textbf{'unused B 147 400'} geeft alle hosts weer die 400 dagen offline zijn, \textbf{'unused\_rmhost B 147 400'} geeft alle commando's om diezelfde hosts te verwijderen. Om dit werkende te krijgen worden de \acrshort{ARP}-tabellen van alle routers binnen UGent meerdere keren per dag uitgelezen. Als een toestel verbinding maakt met het netwerk zal het MAC-adres van dit toestel steeds voorkomen in deze tabel. Als het MAC-adres van een host uit subnet B147 dus 400 dagen niet is voorgekomen in een van de ARP-tabellen, dan zal deze weergegeven worden met dit commando.
\end{itemize}
...
Eens de testen geslaagd zijn voor de drie scripts uit sectie \ref{premigratie} zal het mogelijk zijn om EfficientIP synchroon mee te laten draaien naast de productie-omgeving. Eens dit in orde is het mogelijk om de website te implementeren van waaruit registraties gemaakt kunnen worden die naar EfficientIP gaan. Hoe de implementatie van de website zal gebeuren is terug te vinden in het volgende hoofdstuk \ref{ch:methodologie}.
